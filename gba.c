...
void setPixel(int x, int y, const u16 color) {
    // TA-TODO: IMPLEMENT
...
void drawRectDMA(int x, int y, int width, int height, volatile u16 color) {
    ...
    for (int i = 0; i < height; i++) {
        ...
        DMA[3].cnt = DMA_ON | width | DMA_DESTINATION_INCREMENT | DMA_SOURCE_FIXED | DMA_CHANNEL_3;
    }
}
...
void drawFullScreenImageDMA(const u16 *image) {
    ...
    DMA[3].cnt = DMA_ON | 240*160 | DMA_DESTINATION_INCREMENT | DMA_SOURCE_INCREMENT | DMA_CHANNEL_3;
...
void fillScreenDMA(volatile u16 color) {
    ...
    DMA[3].src = &color;
    DMA[3].dst = videoBuffer;
    DMA[3].cnt = DMA_ON | 240*160 | DMA_DESTINATION_INCREMENT | DMA_SOURCE_FIXED | DMA_CHANNEL_3;
}
